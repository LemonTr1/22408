...
5. **Obsidian 正在运行时，优先走 CLI**（见下节）：读写、属性、任务、链接检查都能同步索引，避免直接改文件导致缓存不一致。
...
| 用命令行操作库（读写/搜索/属性/任务/链接检查），Obsidian 正在运行 | `references/cli-commands.md` |
...
库已在 Obsidian 中打开时，用 `obsidian <command>` 操作，**不要**直接读写 `.md`：

```bash
obsidian read path="01-总览/主页.md"          # 读
obsidian create path="03-数据结构/新笔记.md" content="# 标题"   # 建
obsidian search query="Cache"                 # 搜
obsidian property:set path="..." name="mastery" value="已掌握"  # 改属性
obsidian unresolved total                     # 查死链 ← 收尾必做
```

三条铁律：
1. **用 `path=` 而非 `file=`**——`file=` 按名称模糊匹配，重名时会命中错误笔记。
2. **`move` 不会创建目标目录**，目标不存在直接报 `ENOENT`。
3. **任务 `line=N` 是含 frontmatter 的绝对行号**，先 `tasks verbose` 取行号。

完整命令清单与实测踩坑表见 `references/cli-commands.md`。
